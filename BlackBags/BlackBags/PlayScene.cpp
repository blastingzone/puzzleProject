...
void CPlayScene::CreatePlayers()
{
	...
	while(playerId<m_PlayerNumber)
	{
		while(true)
		{
			tempTurn = rand()%m_PlayerNumber;
			bool flag = false;
			for(int j = 0; j<playerId;++j)
			{
				if(tempTurn == CGameData::GetInstance()->GetPlayerTurn(j))
				{
					flag = true;
					break;
				}
		...
		if((playerMask>>characterId) &1)
		{
			m_Player[tempTurn] = new CPlayer;
			m_Player[tempTurn]->SetPlayerId(playerId);
			m_Player[tempTurn]->SetPlayerName(CGameData::GetInstance()->GetPlayerName(playerId));
			CGameData::GetInstance()->SetPlayerCharacterId(playerId,characterId);
			CGameData::GetInstance()->SetPlayerTurn(playerId,tempTurn);
			++playerId;
		}
		++characterId;
	}
