...
bool CGameMap::Init()
{
	...
	CreateMap();
  	...
  	m_Sprite = new CAnimationRenderer(L"Resource/Image/update/PLAY_gold_ani.png");
  	if (m_Sprite->LoadAnimationImage(80.0f,80.0f,0.3f,S_LT_ONCE) != true )
  		return false;
		...
		for (int j = 0; j <= MAX_MAP_HEIGHT; ++j)
		{
			if (GetMapType(IndexedPosition(i, j) ) == MO_TILE)
			{
				...
				case MO_NOBODY:
					...
					break;
				case MO_PLAYER1:
					tempTileBrush = m_PlayerTurnTable[0]->GetPlayerBrush();
					break;
				case MO_PLAYER2:
					tempTileBrush = m_PlayerTurnTable[1]->GetPlayerBrush();
					break;
				case MO_PLAYER3:
					tempTileBrush = m_PlayerTurnTable[2]->GetPlayerBrush();
					break;
				case MO_PLAYER4:
					tempTileBrush = m_PlayerTurnTable[3]->GetPlayerBrush();
					break;
				default:
					break;
				}
