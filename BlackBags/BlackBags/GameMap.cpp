...
CGameMap::CGameMap(MapSize mapSize)
{
	...
	memset(m_pTileP,0,sizeof(m_pTileP) );
		...
		for (int j = 0; j <= MAX_MAP_HEIGHT; ++j)
		{
			if (GetMapType(IndexedPosition(i, j) ) == MO_TILE)
			{
				...
				case MO_NOBODY:
					...
					break;
				case MO_PLAYER1:
					m_pRenderTarget->FillRectangle(rectElement, m_pTileP[0]);
					break;
				case MO_PLAYER2:
					m_pRenderTarget->FillRectangle(rectElement, m_pTileP[1]);
					break;
				case MO_PLAYER3:
					m_pRenderTarget->FillRectangle(rectElement, m_pTileP[2]);
					break;
				case MO_PLAYER4:
					m_pRenderTarget->FillRectangle(rectElement, m_pTileP[3]);
					break;
				default:
					break;
				}
...
bool CGameMap::CreateResource()
{
	...
	if (m_pRenderTarget == nullptr)
	{
		...
		{
			if(SUCCEEDED(hr))
				hr = m_pRenderTarget->CreateSolidColorBrush(CGameData::GetInstance()->GetPlayerColor(idx), &m_pTileP[idx]);
			if(SUCCEEDED(hr))
			{
				int turn = CGameData::GetInstance()->GetPlayerTurn(idx);
