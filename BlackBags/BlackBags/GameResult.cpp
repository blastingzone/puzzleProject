...
bool CGameResult::CreateResource()
{
	...
	if (m_pRenderTarget == nullptr)
	{
		...
		if (SUCCEEDED(hr) )
			hr = m_pRenderTarget->CreateSolidColorBrush(D2D1::ColorF(_COLOR_CHARACTER_1_), &m_TileBrush[0]);

		if (SUCCEEDED(hr) )
			hr = m_pRenderTarget->CreateSolidColorBrush(D2D1::ColorF(_COLOR_CHARACTER_2_), &m_TileBrush[1]);

		if (SUCCEEDED(hr) )
			hr = m_pRenderTarget->CreateSolidColorBrush(D2D1::ColorF(_COLOR_CHARACTER_3_), &m_TileBrush[2]);

		if (SUCCEEDED(hr) )
			hr = m_pRenderTarget->CreateSolidColorBrush(D2D1::ColorF(_COLOR_CHARACTER_4_), &m_TileBrush[3]);
